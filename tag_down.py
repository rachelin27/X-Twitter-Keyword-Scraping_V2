...
proxy = ''
# 手动配置代理,非必要无需填写 格式: http://localhost:port

tag = 'Trump'
_filter = 'since:2026-04-22 until:2026-04-26'
# 填入tag 可留空
# _filter = "since:2026-04-22 until:2026-04-26"
# (可选项) 高级搜索
...
if text_down:
    ...
    if media_latest:
        ...
        product = 'Latest'
        mode = 'media_latest'
query = ' '.join(part for part in [tag.strip(), _filter.strip()] if part)
            ...
            while True:
                try:
                    async with semaphore:
                        if proxy:
                            client = httpx.AsyncClient(proxy=proxy, trust_env=False)
                        else:
...
class tag_down():
    def __init__(self):
        ...
        referer_tab = 'live' if text_down or media_latest else 'media'
        self._headers['referer'] = f'https://twitter.com/search?q={quote(query)}&src=typed_query&f={referer_tab}'

        self.cursor = ''

        self.ct = get_transaction_id(proxy)

        try:
            for i in range(down_count//entries_count):
                url = 'https://x.com/i/api/graphql/AIdc203rPpK_k_2KWSdm7g/SearchTimeline?variables={"rawQuery":"' + quote(query) + '","count":' + str(entries_count) + ',"cursor":"' + self.cursor + '","querySource":"typed_query","product":"' + product + '"}&features={"rweb_video_screen_enabled":false,"profile_label_improvements_pcf_label_in_post_enabled":true,"rweb_tipjar_consumption_enabled":true,"verified_phone_label_enabled":false,"creator_subscriptions_tweet_preview_api_enabled":true,"responsive_web_graphql_timeline_navigation_enabled":true,"responsive_web_graphql_skip_user_profile_image_extensions_enabled":false,"premium_content_api_read_enabled":false,"communities_web_enable_tweet_community_results_fetch":true,"c9s_tweet_anatomy_moderator_badge_enabled":true,"responsive_web_grok_analyze_button_fetch_trends_enabled":false,"responsive_web_grok_analyze_post_followups_enabled":true,"responsive_web_jetfuel_frame":false,"responsive_web_grok_share_attachment_enabled":true,"articles_preview_enabled":true,"responsive_web_edit_tweet_api_enabled":true,"graphql_is_translatable_rweb_tweet_is_translatable_enabled":true,"view_counts_everywhere_api_enabled":true,"longform_notetweets_consumption_enabled":true,"responsive_web_twitter_article_tweet_consumption_enabled":true,"tweet_awards_web_tipping_enabled":false,"responsive_web_grok_show_grok_translated_post":false,"responsive_web_grok_analysis_button_from_backend":false,"creator_subscriptions_quote_tweet_preview_enabled":false,"freedom_of_speech_not_reach_fetch_enabled":true,"standardized_nudges_misinfo":true,"tweet_with_visibility_results_prefer_gql_limited_actions_policy_enabled":true,"longform_notetweets_rich_text_read_enabled":true,"longform_notetweets_inline_media_enabled":true,"responsive_web_grok_image_annotation_enabled":true,"responsive_web_enhance_cards_enabled":false}'
    ...
    def search_media(self, url):
        ...
        if proxy:
            response = httpx.get(url, headers=self._headers, proxy=proxy, trust_env=False).text
        else:
            response = httpx.get(url, headers=self._headers, trust_env=False).text
        try:
